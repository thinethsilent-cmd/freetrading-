...
const APP = {
    ...
    currentMarket: 'crypto', // 'crypto' or 'forex'
    forexData: [
        { id: 'gold', symbol: 'xauusd', name: 'Gold', current_price: 2185.40, price_change_percentage_24h: 0.45, image: 'https://cdn-icons-png.flaticon.com/512/272/272531.png' },
        { id: 'eurusd', symbol: 'eurusd', name: 'EUR/USD', current_price: 1.0894, price_change_percentage_24h: -0.12, image: 'https://cdn-icons-png.flaticon.com/512/197/197374.png' },
        { id: 'gbpusd', symbol: 'gbpusd', name: 'GBP/USD', current_price: 1.2735, price_change_percentage_24h: 0.08, image: 'https://cdn-icons-png.flaticon.com/512/197/197374.png' },
        { id: 'audusd', symbol: 'audusd', name: 'AUD/USD', current_price: 0.6542, price_change_percentage_24h: 0.21, image: 'https://cdn-icons-png.flaticon.com/512/197/197374.png' },
        { id: 'usdjpy', symbol: 'usdjpy', name: 'USD/JPY', current_price: 147.12, price_change_percentage_24h: -0.34, image: 'https://cdn-icons-png.flaticon.com/512/197/197374.png' }
    ]
};
...
async function fetchCryptoData() {
    try {
        const res = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=200&page=1&sparkline=false&price_change_percentage=24h');
...
function initMarketTabs() {
    document.querySelectorAll('.market-tab').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            btn.classList.add('active');
            APP.currentMarket = btn.dataset.market;
            const data = APP.currentMarket === 'crypto' ? APP.cryptoData : APP.forexData;
            renderCryptoTable(applyFilter(data, APP.currentFilter));
        });
    });
}
...
function updateGoalProgress() {
    ...
    if (!totalEl) return;
    const totalValue = parseFloat(totalEl.textContent.replace('$', '').replace(',', '')) || 0;
    const goalValue = 24500; // BMW S1000RR Price
    const progress = Math.min((totalValue / goalValue) * 100, 100);
    ...
    if (status) {
        if (progress >= 100) {
            status.innerHTML = '<span class="status-ready">READY TO EXCHANGE <i class="fa-solid fa-motorcycle"></i></span>';
            showToast('BMW S1000RR GOAL REACHED! 🏁', 'success');
        } else {
            const remaining = goalValue - totalValue;
            status.textContent = `$${formatNumber(remaining, 0)} remaining to S1000RR`;
        }
    }
}
...
function renderChart(symbol, interval) {
    ...
    if (APP.currentMarket === 'forex') {
        if (symbol === 'XAUUSD') tvSymbol = 'SAXO:XAUUSD';
